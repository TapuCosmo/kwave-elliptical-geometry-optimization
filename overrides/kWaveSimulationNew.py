"""
This file was derived from # https://github.com/waltsims/k-wave-python/blob/3f004cfc5871613c5fa5e96bd3686f7da2527680/kwave/kWaveSimulation.py.
...
from kwave.utils.conversion import cast_to_type, cart2grid, grid2cart
...
class kWaveSimulation(object):
    ...
    def __init__(self,
        ...
        if self.sensor.time_reversal_boundary_data is not None:
            ...
            if hasattr(self.sensor, "order_index"):
              self.order_index = self.sensor.order_index
            if hasattr(self.sensor, "reorder_index"):
              self.reorder_index = self.sensor.reorder_index
    ...
    def check_sensor(self, kgrid_dim)-> None:
        ...
        if self.sensor is not None:
            ...
            #     new_col_pos = len(self.source.p[0, :])
            #     self.source.p = np.append(self.source.p, self.reorder_index.flatten()[..., None], 1)
            #     # self.source.p[:, new_col_pos] = self.order_index

            #     print(self.source.p)

            #     # reorder p0 based on the order_index
            #     # self.source.p = sort_rows(self.source.p, new_col_pos);
            #     self.source.p = self.source.p[self.source.p[:, new_col_pos].argsort()]

            #     print(self.source.p)

            #     # remove the reordering data
            #     # self.source.p = self.source.p[:, 0:new_col_pos];
            #     self.source.p = np.delete(self.source.p, new_col_pos, 1)
            #     # """
